  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_LOVAC_BACKEND_URL}/api/new-message`, {
        ...
        body: JSON.stringify({ staffId, ticketId: Number(ticketId), body: inputValue })
      })
      ...
      <AnimatePresence>
        {showUnassignAlert && (
          <motion.div
            ...
            exit={{ opacity: 0, x: 50 }}
            className="fixed bottom-4 right-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded flex items-center"
            role="alert"
          >
            <CheckCircle className="h-6 w-6 mr-2" />
            <span>Ticket unassigned successfully</span>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
